...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const themeToggle = document.getElementById('hubThemeToggle');
  const savedTheme = localStorage.getItem('conectTattooTheme') || 'light';
  ...
  if (themeToggle) {
    themeToggle.addEventListener('click', () => {
      ...
      localStorage.setItem('conectTattooTheme', next);
    });
  }
  ...
  // ── Widget de Pontos (mockup visual, sem persistência) ─
  ...
  pointsToggle.addEventListener('click', () => {
    pointsPanel.classList.toggle('open');
  });
  pointsClose.addEventListener('click', () => {
    pointsPanel.classList.remove('open');
  });
  document.addEventListener('click', (e) => {
    if (!document.getElementById('pointsWidget').contains(e.target)) {
      pointsPanel.classList.remove('open');
    }
  });
  ...
  document.querySelectorAll('.share-btn').forEach(btn => {
    btn.addEventListener('click', () => {
      ...
      // NOTA: aqui entraria a lógica real de compartilhamento
      // (abrir URL de share do WhatsApp/Instagram/TikTok) e,
      // depois de confirmado o compartilhamento, somar pontos
      // de verdade (hoje é só visual).
      showToast(`Compartilhamento via ${network} em breve disponível!`);
    });
  });
