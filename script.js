...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // authToken: chave usada pra manter o usuário conectado automaticamente
  // entre visitas (inclusive ao abrir o app instalado pelo atalho da PWA).
  // TODO no futuro: troque o valor salvo aqui pelo token real vindo do backend.
  ...
  // Login persistente (authToken): se o usuário já tem um token salvo,
  // ele entra direto — sem passar pela tela de login de novo — e cai
  // automaticamente na seção Meu Perfil. Isso tem prioridade sobre a
  // flag de testes acima, porque é exatamente o comportamento de produção
  // que a persistência de login precisa garantir (inclusive pra PWA/atalho).
  if (getAuthToken()) {
    reflectAccessInUI();
    gate.classList.add('gate-hidden');
    document.body.style.overflow = '';
    document.body.classList.remove('gate-locked');
    goToPerfilHub();
  } else if (!SEMPRE_MOSTRAR_GATE_EM_TESTES && getAccessType()) {
    reflectAccessInUI();
    gate.classList.add('gate-hidden');
  } else {
  ...
  if (gateForm) {
    gateForm.addEventListener('submit', (e) => {
      e.preventDefault();
      enterAsMember(true); // login com email/senha: mantém conectado (salva authToken)
    });
  }
  if (gateSignup) gateSignup.addEventListener('click', () => enterAsMember(true));
  if (gateInstagram) gateInstagram.addEventListener('click', () => enterAsMember(true));
